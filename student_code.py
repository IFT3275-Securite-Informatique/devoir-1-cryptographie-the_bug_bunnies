...
sortedSymbolesRatio = [(symbole, valeur / symbolTotal) for symbole, valeur in occurencesSymboles]
#print(symbolesRatio)
...
def decrypt(C):
    ...
    symbolBitsTotal = sum(valeur for _, valeur in tupleBits)
    sortedSymbolesBitsRatio = [(symbole, valeur / symbolBitsTotal) for symbole, valeur in tupleBits]

    #modifier ça pour aller vers des proba
    dictChiffreVersLettre = dictSymbolesRatio(sortedSymbolesBitsRatio, sortedSymbolesRatio)
    #dict(list(zip(dictToutBits, dictToutSymbol)))

    print("dict chiffre vers lettre: ", dictChiffreVersLettre)
...
#fonction qui prend en param le dictionnaire de Bits + occurence et
#le dictionnaire de symboles avec le ratio, puis les mappe avec la probab la + proche
def dictSymbolesRatio(dictToutBits, sortedSymbolesRatio):

   return None

def dictSymbolesRatio(sortedBitsSymbolesRatio, sortedSymbolesRatio):
   finalDict = {}
#faire une recherche dichotomique si c'est trop long pour les 90 000 chars.
   for i in range(len(sortedBitsSymbolesRatio)):    #match ch bits
      for j in range(len(sortedSymbolesRatio)):     #passe travers symboles

         if(sortedBitsSymbolesRatio[i][1]>sortedSymbolesRatio[j][1]):
            if(j==0):
                finalDict[sortedBitsSymbolesRatio[i][0]] = sortedSymbolesRatio[0][0]
            else:
                if(abs(sortedBitsSymbolesRatio[i][1]-sortedSymbolesRatio[j][1])
                   < abs(sortedBitsSymbolesRatio[i][1]-sortedSymbolesRatio[j-1][1])):
                        finalDict[sortedBitsSymbolesRatio[i][0]] = sortedSymbolesRatio[j][0]
                else:
                   finalDict[sortedBitsSymbolesRatio[i][0]] = sortedSymbolesRatio[j-1][0]
